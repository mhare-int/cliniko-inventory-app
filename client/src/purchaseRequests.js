...
function PurchaseRequests() {
  ...
  if (include && receivedQty > 0 && productName) {
      ...
      // Check for items with outstanding quantities after receiving
      ...
      items.forEach((item, idx) => {
        ...
        const receivedSoFar = item["received_so_far"] ?? item.received_so_far ?? 0;
        const totalReceived = (receivedSoFar + receivedQty);
        if (totalReceived < ordered) {
          outstandingItems.push({ item, idx, totalReceived, ordered, productName: item["Product Name"] ?? item.name, prId: item.pr_id, productId: item.product_id ?? item.productId ?? item.Id ?? item.id ?? null });
        }
      });

      if (outstandingItems.length > 0) {
        const productNames = outstandingItems.map(o => `• ${o.productName} (received ${o.totalReceived} of ${o.ordered})`).join('\n');
        const deleteRemaining = window.confirm(
          `The following items were not fully received:\n\n${productNames}\n\nClick OK to DELETE the remaining quantities and close these lines.\nClick Cancel to keep them on BACKORDER.`
        );
        if (deleteRemaining) {
          // Close outstanding lines by setting no_to_order = totalReceived
          for (const o of outstandingItems) {
            try {
              const changedBy = await resolveCurrentUserIdentifier();
              await window.api.updatePurchaseRequestItemsEditWithComment(
                o.prId,
                [{ productName: o.productName, no_to_order: o.totalReceived }],
                changedBy,
                'Remaining quantities deleted when receiving vendor order'
              );
            } catch (editErr) {
              console.error('Failed to close outstanding line:', editErr);
            }
          }
          message += `\n🗑️ Deleted remaining quantities for ${outstandingItems.length} item(s).`;
        } else {
          message += `\n📋 ${outstandingItems.length} item(s) kept on backorder.`;
        }
      }
