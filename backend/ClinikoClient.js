...
class ClinikoClient {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = httpModule.request(options, (res) => {
        ...
        res.on('end', async () => {
          ...
          // Replace large integers (>15 digits) with quoted strings BEFORE
          // JSON.parse to prevent JavaScript losing precision on Cliniko's
          // 64-bit product IDs (e.g. 1213533212211414143 → 1213533212211414000).
          try {
            const safeData = data.replace(/:(\s*)(\d{16,})([,\}\]])/g, ':$1"$2"$3');
  ...
  async fetchProducts(options = {}) {
    ...
    const allProducts = await this.fetchAllPages('/products?per_page=100', onProgress);
